...
const TAG_STYLE = "px-4 py-2.5 rounded-full bg-white/20 backdrop-blur-xl border border-white/60 text-slate-700 text-sm font-medium whitespace-nowrap shadow-[0_4px_24px_rgba(0,0,0,0.12),0_0_0_1px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,0.6)]";

export default function HeroSection() {
  const topRow = RESEARCH_AREAS.slice(0, 4);
  const bottomRow = RESEARCH_AREAS.slice(4, 7);
        ...
        <div className="flex flex-col items-center gap-3 mt-6 px-6">
          <div className="flex flex-wrap justify-center gap-3">
            {topRow.map((area, index) => (
              <span key={index} className={TAG_STYLE}>{area}</span>
            ))}
          </div>
          <div className="flex flex-wrap justify-center gap-3">
